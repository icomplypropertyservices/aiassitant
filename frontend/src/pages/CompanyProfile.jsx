  ...
  return (
    <PageShell style={{ overflowX: 'hidden' }}>
      <Space direction="vertical" size={16} style={{ width: '100%' }}>
        ...
        {editing && (
          <Card title="Edit company" size="small" className="aba-soft-card">
        ...
        {!editing && profile.notes && (
          <Card title="Notes" size="small" className="aba-soft-card">
            <Paragraph type="secondary" style={{ marginBottom: 0 }}>{profile.notes}</Paragraph>
          </Card>
        )}

        {/* Overview KPIs */}
        <Card title="Overview" size="small" className="aba-soft-card">
          <Row gutter={[12, 12]}>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Projects" value={profile.project_count ?? profile.projects?.length ?? 0} /></Card></Col>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Agents" value={profile.agents?.length ?? 0} prefix={<RobotOutlined />} /></Card></Col>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Humans" value={profile.humans?.length ?? 0} prefix={<UserOutlined />} /></Card></Col>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Customers" value={s.customers ?? 0} /></Card></Col>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Products" value={s.products ?? 0} /></Card></Col>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Pipeline open" prefix="$" value={s.pipeline_open_value ?? 0} precision={0} /></Card></Col>
            <Col xs={12} sm={8} md={4}><Card size="small" type="inner"><Statistic title="Won revenue" prefix="$" value={s.pipeline_won_value ?? 0} precision={0} valueStyle={{ color: '#52c41a' }} /></Card></Col>
          </Row>
        </Card>

        {/* Finance KPIs */}
        <Card title="Finance" size="small" className="aba-soft-card">
          <Row gutter={[12, 12]}>
            <Col xs={12} md={6}>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={14}>
            <Card title={`AI cost & tokens (${days} days)`} size="small" className="aba-soft-card">
              {daily.length === 0 ? (
          ...
          <Col xs={24} lg={10}>
            <Card title="Pipeline value" size="small" className="aba-soft-card">
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} md={12}>
            <Card title="P&L snapshot" size="small" className="aba-soft-card">
          ...
          <Col xs={24} md={12}>
            <Card title="AI cost by model" size="small" className="aba-soft-card">
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={12}>
            <Card title="Agents" size="small" className="aba-soft-card" extra={<Link to="/agents">All agents</Link>}>
              <Table
                size="small"
                rowKey="id"
        ...
        <Card title="Projects" size="small" className="aba-soft-card">
          <Table
            size="small"
            rowKey="id"
